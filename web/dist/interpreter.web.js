...
export class MeowJSInterpreter {
    ...
    isValidVariableName(token) {
        return (!this.isStringLiteral(token) &&
            !(commandMap[token.toLowerCase()] ?? commandMap[token]));
    }
    handleVariableSet() {
        if (this.stack.length < 2) {
            throw new Error("Stack underflow for 'nest'. Expected: value variableName nest");
        }
    ...
    handleVariableGet() {
        if (this.stack.length < 1) {
            throw new Error("Stack underflow for 'fetch'. Expected: variableName fetch");
        }
        const varName = this.stack.pop();
        if (typeof varName !== "string" || !this.isValidVariableName(varName)) {
            throw new Error(`Invalid variable name: ${varName}`);
    ...
    handleArithmetic(operation) {
        if (this.stack.length < 2) {
            throw new Error("Stack underflow for arithmetic operation. Expected: operand1 operand2 operation");
        }
